...
object TaxiYellow {
  val spark = SparkSession.builder().appName("tlcYellowTaxiETL").getOrCreate()
  ...
  def main(args: Array[String]): Unit = {
    ...
    val filePaths2020 = for (m <- months.slice(9, 12)) yield s"$sourcePath/2020/$m/yellow_tripdata_2020-${m}"
    val filePaths2021 = for (m <- months) yield s"$sourcePath/2021/${m}/yellow_tripdata_2021-${m}"
    val filePaths2022 = for (m <- months) yield s"$sourcePath/2022/${m}/yellow_tripdata_2022-${m}"
    val filePaths2023 = for (m <- months.slice(0, 9)) yield s"$sourcePath/2023/${m}/yellow_tripdata_2023-${m}"
